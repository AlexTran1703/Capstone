...
def save_data():
    ...
    output = processecg.predict(signal=heartbeats, file='LSTM.h5')
...
def document_plot(id):
    ...
    if 'ECGsignal' in document:
        ...
        for i in range(0,len(output)):
            ...
            if output[i] == 1:
                ecg_plot = ecgdata[num_beat[i]-a: num_beat[i]+b]
                color = 'red'
            ...
            try:
                if color != 'blue': 
                    ax.plot(n, ecg_plot, color = color)
                color = 'blue'
                break
            except ValueError:
                c = 20
                d = 40
                n = [j for j in range(num_beat[i] - c, num_beat[i] + d)]
                ecg_plot = ecgdata[num_beat[i]-c: num_beat[i]+d]
                ax.plot(n, ecg_plot, color = color)
            except:
                color = 'blue'
